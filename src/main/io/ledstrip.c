...
static void applyLedFixedLayers()
{
    for (int ledIndex = 0; ledIndex < ledCounts.count; ledIndex++) {
        ...
        if (ledGetOverlayBit(ledConfig, LED_OVERLAY_THROTTLE)) {
			color.h = scaleRange(rcData[THROTTLE], PWM_RANGE_MIN, PWM_RANGE_MAX, color.h, color2.h);  //Fade the color smoothly from color to the next color from min to max throttle
			color.s = scaleRange(rcData[THROTTLE], PWM_RANGE_MIN, PWM_RANGE_MAX, color.s, color2.s);
			color.v = scaleRange(rcData[THROTTLE], PWM_RANGE_MIN, PWM_RANGE_MAX, color.v, color2.v);
			hOffset = 0; 
        }
